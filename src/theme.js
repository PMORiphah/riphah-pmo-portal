...
export const STAGE_META = {
  // Coral rather than neutral: "closed" also uses DATA.neutral, so the two were
  // rendered in an identical grey-blue and were indistinguishable in the
  // pipeline bar, the donut and the legend.
  pdd_not_submitted:{ label:"PDD Not Submitted", color:DATA.danger,   short:"Not Submitted" },
...
export const healthOf = (cpi, spi) => {
  ...
  if (!has(cpi) && !has(spi))
    return { key:"nodata", label:"Insufficient Data", color:DATA.info,
...
export const KPI_INSIGHT = {
  ...
  // PCD is not a widely known abbreviation, so the tooltip spells it out
  // rather than assuming the reader already knows.
  pcds_received: (d) => `PCD stands for Project Closing Document — submitted when a project is completed and handed over. ${d?.closed_count ?? 0} of ${d?.total_projects ?? 0} projects have one on record.`,
...
export const KPI_INSIGHT_PLAIN = {
  ...
  pcds_received: () => "PCD stands for Project Closing Document — submitted when a project is completed and handed over.",
